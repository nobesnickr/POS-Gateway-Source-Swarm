...
namespace Diagnostic
{
    ...
    public class SwarmService
    {
        ...
        public SwarmService(Uri baseAddress, SecurityProtocolType securityProtocol)
        {
            this.swarmId = "diagnostics";
            this.posSoftwareId = "retailpro8";
        ...
        public Task UploadAsync(string url, object content)
        {
            ...
            return Task.Factory.StartNew(() =>
            {
                ...
                try
                {
                    ...
                    Logger.Info("Uploading data to {0} ({1})", url, this.DebugInfo);
                ...
                finally
                {
                    if (response != null)
                    {
                        Logger.Warn("Response for {0} with was {1} - {2}", url, response.StatusCode, response.StatusDescription);
                    }
                }
            });
        }
        ...
        public Task<string> GetAsync<TResult>(string url)
        {
            ...
            return Task.Factory.StartNew<string>(() =>
            {
                ...
                {
                    ServicePointManager.SecurityProtocol = this.securityProtocol;
                    Logger.Trace("Downloading data from {0} ({1})", url, this.DebugInfo);
        ...
        {
            Logger.Debug("CreateUploadRequest({0},{1},{2}) called", url, jsonContent, compress);
            ...
            Logger.Debug("HttpWebRequest instance created.");
            return request;
        }
    }
}
