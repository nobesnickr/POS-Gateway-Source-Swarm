...
package hu.sonrisa.backend.org.apache.tools.zip;
   ...
   // CheckStyle:VisibilityModifier OFF - bc
    ...
    public void closeEntry() throws IOException {
        ...
        if (entry.getMethod() == DEFLATED) {
            ...
            if (entry.getCrc() != realCrc) {
                throw new ZipException(BackendExceptionConstants.BEND_00035
                                       + entry.getName() + ": "
                                       + Long.toHexString(entry.getCrc())
                                       + BackendExceptionConstants.BEND_00041
                                       + Long.toHexString(realCrc));
            }

            if (entry.getSize() != written - dataStart) {
                throw new ZipException(BackendExceptionConstants.BEND_00036
                                       + entry.getName() + ": "
                                       + entry.getSize()
                                       + BackendExceptionConstants.BEND_00041
                                       + (written - dataStart));
    ...
    public void putNextEntry(ZipEntry ze) throws IOException {
        ...
        if (entry.getMethod() == STORED && raf == null) {
            if (entry.getSize() == -1) {
                throw new ZipException(BackendExceptionConstants.BEND_00037);
            }
            if (entry.getCrc() == -1) {
                throw new ZipException(BackendExceptionConstants.BEND_00038);
            }
    ...
    public void setLevel(int level) {
        if (level < Deflater.DEFAULT_COMPRESSION
            ...
            throw new IllegalArgumentException(BackendExceptionConstants.BEND_00034
                                               + level);
        }
    ...
    protected byte[] getBytes(String name) throws ZipException {
        try {
            ...
            throw new ZipException(BackendExceptionConstants.BEND_00038 + ex.getMessage());
        }
    }
