...
public abstract class AbstractUnicodeExtraField implements ZipExtraField {
    ...
    protected AbstractUnicodeExtraField(String text, byte[] bytes, int off,
        ...
        try {
            ...
            throw new RuntimeException(BackendExceptionConstants.BEND_00018,
                                       e);
        }
    }
    ...
    public void parseFromLocalFileData(byte[] buffer, int offset, int length)
        ...
        if (length < 5) {
            throw new ZipException(BackendExceptionConstants.BEND_00019);
        }

        int version = buffer[offset];

        if (version != 0x01) {
            throw new ZipException(BackendExceptionConstants.BEND_00020 + version);
        }
